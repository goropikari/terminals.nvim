...
---@field tabpages table<integer, TerminalsTabState>
...
function M.prune()
  local valid = valid_tabpages()
  for tabpage in pairs(state.tabpages) do
    if not valid[tabpage] then
      state.tabpages[tabpage] = nil
    end
  end
end
...
function M.get_tab(tabpage)
  M.prune()
  tabpage = tabpage or M.current_tabpage()
  local tab = state.tabpages[tabpage]
  if not tab then
    tab = {
      terminals = {},
      active_id = nil,
